...
enum { AUTORECEIVE_SWITCH_ID = 0, SETTINGS_SWITCHES_NB };
...
static void autoreceive_warning_choice(bool confirm) {
    ...
    if (confirm) {
        ...
        switch_value = !N_storage.autoreceive_allowed;
        switches[AUTORECEIVE_SWITCH_ID].initState = (nbgl_state_t) switch_value;
        // store the new setting value in NVM
        nvm_write((void *) &N_storage.autoreceive_allowed, &switch_value, 1);
    }
...
static void controls_callback(int token, uint8_t index, int page) {
    ...
    uint8_t switch_value;
    if (token == AUTORECEIVE_SWITCH_TOKEN) {
        // Auto-receive switch touched, display a warning when the user wants
        // to activate the setting
        if (!N_storage.autoreceive_allowed) {
...
void ui_menu_main(void) {
    ...
    switches[AUTORECEIVE_SWITCH_ID].initState = (nbgl_state_t) N_storage.autoreceive_allowed;
    switches[AUTORECEIVE_SWITCH_ID].text = "Auto-receive";
    switches[AUTORECEIVE_SWITCH_ID].subText = "Sign receivables without input";
    switches[AUTORECEIVE_SWITCH_ID].token = AUTORECEIVE_SWITCH_TOKEN;
#ifdef HAVE_PIEZO_SOUND
    switches[AUTORECEIVE_SWITCH_ID].tuneId = TUNE_TAP_CASUAL;
#endif
